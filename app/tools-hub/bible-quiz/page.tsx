...
const CHARACTERS = [
  ...
  { id: 'solomon', name: '所羅門', title: '智慧之王', desc: '任何題目答對，基礎傷害 x 1.2', img: 'https://mc-heads.net/body/Solomon/256' },
  ...
  { id: 'paul', name: '保羅', title: '外邦使徒', desc: '答對「聖經歷史」與「猜人物」時，傷害 x 1.2', img: 'https://mc-heads.net/body/Paul/256' }
];
...
export default function BibleQuiz() {
  ...
  const handleCharSelect = (charId: string) => {
    if (gameState === 'selectA') { setTeamAChar(charId); setGameState('selectB'); } 
    else if (gameState === 'selectB') { setTeamBChar(charId); setGameState('reveal'); }
  };
  ...
  const checkWinCondition = () => {
    setHp(currentHp => {
      if (currentHp.teamA <= 0 || currentHp.teamB <= 0 || currentIndex + 1 >= questions.length) {
        setGameState('end');
      } else {
  ...
  // 🌟 全面縮小了 padding 和 border，以適應 1080p/768p 螢幕
      ...
      <div className="w-full h-full bg-slate-300 relative flex flex-col items-center justify-center p-4 md:p-6 border-x-[8px] md:border-x-[12px] border-slate-900 overflow-hidden">
        ...
        {(gameState === 'selectA' || gameState === 'selectB') && (
           ...
           <div className="grid grid-cols-5 gap-3 md:gap-5">
             {CHARACTERS.map(char => (
               <button key={char.id} onClick={() => handleCharSelect(char.id)} disabled={gameState === 'selectB' && teamAChar === char.id} className={`${pixelBoxClass} flex flex-col items-center justify-center hover:bg-slate-700 p-2 md:p-4`}>
